...
function Home() {
  ...
  return (
    <div className="min-h-screen text-white flex flex-col items-center overflow-y-auto bg-black" style={{ backgroundImage: "url('/desktop/bu.webp')", backgroundSize: "cover", backgroundPosition: "center" }}>
      <header className="w-full bg-black py-6 fixed top-0 shadow-2xl z-50 flex justify-between items-center px-10">
        <div className="flex items-center text-xl text-white">
          <FaUserCircle className="mr-2 text-green-400 text-3xl" />
          <span className="font-semibold tracking-widest">Portfolio</span>
        </div>
        <nav className="flex space-x-6">
          {[{ id: "#home", title: "Əsas" }, { id: "#projects", title: "Layihələr" }, { id: "#experience", title: "Təcrübə" }, { id: "#skills", title: "Bacarıqlar" }, { id: "#about", title: "Haqqında" }, { id: "#contact", title: "Əlaqə" }].map((item) => (
            <motion.button
              ...
              className="text-sm text-white px-3 py-2 relative cursor-pointer overflow-hidden group"
              whileHover={{ scale: 1.1 }}
            >
              {item.title}
              <span className="absolute left-0 bottom-0 w-0 h-0.5 bg-orange-500 transition-all duration-500 group-hover:w-full"></span>
            </motion.button>
          ))}
        </nav>
      </header>
